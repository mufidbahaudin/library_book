...
if (isset($_GET['search'])) {
    ...
    $query = "SELECT * FROM buku WHERE judul_buku LIKE '%$search_keyword%' OR penulis_buku LIKE '%$search_keyword%' OR genre LIKE '%$search_keyword%'";
} else {
...
<head>
    ...
    <link rel="stylesheet" href="css/admin_buku.css">
</head>
...
<body>
    ...
    <div class="container mt-5">
        ...
        <div class="alert alert-success" role="alert">
            Hello, <?= htmlspecialchars($_SESSION['username']) ?>! Anda adalah <strong>ADMIN</strong>, Anda bisa melakukan edit dan hapus buku.
        </div>

        <!-- Form Pencarian -->
        <form action="lihat_buku.php" method="get" class="mb-4">
            <div class="input-group">
                <input type="text" class="form-control" placeholder="Cari buku berdasarkan judul, penulis, atau genre..." name="search" value="<?= htmlspecialchars($search_keyword) ?>">
        ...
        <?php if (isset($_GET['message'])): ?>
            <div class="alert <?= ($_GET['message'] == 'success') ? 'alert-success' : (($_GET['message'] == 'error') ? 'alert-danger' : 'alert-warning') ?>">
                <?= ($_GET['message'] == 'success') ? 'Buku berhasil dihapus!' : (($_GET['message'] == 'error') ? 'Terjadi kesalahan saat menghapus buku!' : 'ID buku tidak valid!') ?>
            </div>
        <?php endif; ?>

        <!-- Tabel Data Buku -->
        <div class="table-responsive">
            <table class="table table-bordered mt-3 table-dark-custom">
                <thead>
                    <tr>
                        <th>ID/Kode Buku</th>
                        <th>Gambar Buku</th>
                    ...
                    <?php if ($result->num_rows > 0): ?>
                        <?php while ($row = $result->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td>
                                    <img src="<?= htmlspecialchars($row['gambar_buku']) ?>" alt="Gambar Buku" class="img-thumbnail" width="100" height="100">
                                </td>
                                ...
                                <td>
                                    <a href="edit_buku.php?id_buku=<?= htmlspecialchars($row['id_buku']) ?>" class="btn btn-warning btn-sm">Edit</a>
                                    <a href="hapus_buku.php?id_buku=<?= htmlspecialchars($row['id_buku']) ?>" class="btn btn-danger btn-sm" onclick="return confirm('Apakah Anda yakin ingin menghapus buku ini?')">Hapus</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    <?php else: ?>
